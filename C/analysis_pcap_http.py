...
for uniquepacket in temppackets:
    if(uniquepacket[0] == 1080):
        packets.append(uniquepacket)
    elif(uniquepacket[1] == 1080):
        sentpacket += 1
    ...
    for templist in uniqueport:
        ...
        if (templist[0] == sourceport and templist[1] == desinationport):  #Sender to Reciever
            ...
            if(templist[3] not in countgetmessagepacket):
                ...
                http10flag = 0
            print("HTTP REQUEST ",templist)
        else:
            if(templist[2] in countgetmessagepacket):
                print("HTTP RESPONSE",templist)
    print("\n")
...
print("The Transmission time for Port: 1080 is",packets[len(packets)-1][7] -packets[0][7])
